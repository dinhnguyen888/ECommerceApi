...
namespace backend.Services
{
    public class PaymentService : IPaymentService
    {
        ...
        public async Task<(List<PaymentGetDto> payments, int totalPayments)> GetPaymentsAsync(int page, int pageSize)
        {
            ...
            var totalPayments = await _context.Payments.CountAsync(); // Ð?m t?ng s? b?n ghi
        ...
        public async Task SendEmailUsingPaymentInfo(PaymentGetDto paymentResult)
        {
            ...
            var subject = "Thanh toán thành công!! Ph?n m?m c?a b?n dã s?n sàng.";
            var body = $@"
                    B?n v?a d?t mua ph?n m?m {paymentResult.ProductPay} v?i giá {paymentResult.ProductPrice} vào ngày {paymentResult.PaymentDate}.
                    Vui lòng click vào du?ng link bên du?i d? t?i:
                    {productURL}";
